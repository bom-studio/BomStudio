...
function paymentTermsByDepositRatio(ratio: 30 | 50) {
  if (ratio === 30) {
...
export function EstimateBuilderForm({ inquiry }: EstimateBuilderFormProps) {
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
        <div className="space-y-6">
              ...
              <AccordionItem value="request">
                ...
                <AccordionContent>
                  <div className="space-y-2 rounded-xl border border-dashed border-border bg-slate-50/60 p-4 text-sm">
                    ...
                    <p><span className="font-medium">기타 요청사항:</span> {inquiry.message || "-"}</p>
                  </div>
                </AccordionContent>
              </AccordionItem>
            </Accordion>
          ...
          <div className="flex items-center justify-between pt-2">
            ...
            <Button asChild>
              <Link
                href={`/admin/estimates/preview?inquiryId=${inquiry.id}&draft=${encodeDraft(draft)}`}
              >
                다음
